...
from run_models_age import ModelKeeper
from run_metrics_age import evaluate_one_emb
...
def run_grid_search(all_hyperparameter_grids, sample_fractions, train_data_in, valid_data_in,
                    ...
                    col_id="client_id", target_col="bins", out_prefix=None,
                    verbose=0, n_samples=10, downstream_type="catboost", devices=0,
                    resume=True, time_limit=23*3600):
    start_time = time()
    all_embeddings = []
    ...
    eval_many_embs(
        ...
        resume=resume, time_limit=time_limit
    )

def eval_many_embs(embs_list, targets, col_id="client_id", target_col="bins",
                   out_prefix=None, sample_fractions=(1/20,), verbose=0,
                   n_samples=10, downstream_type="catboost", resume=True,
                   time_limit=20*3600):
    start_time = time()
    results = defaultdict(list)

    processed_keys = {}
    if resume and out_prefix:
        for sf in sample_fractions:
            csv_path = f"{out_prefix}_{sf:.3f}".rstrip("0").rstrip(".") + ".csv"
            if os.path.exists(csv_path):
                df_existing = pd.read_csv(csv_path)
                hyper_cols = [c for c in df_existing.columns
                              if c not in ['checkpoint','epoch_num','early_stop_epoch',
                                           'recall_topk','accuracy','roc_auc','sample_fraction']
                              and not c.startswith('metric_')
                              and not c.startswith('std_')]
                keys = set()
                for _, row in df_existing.iterrows():
                    keys.add(tuple(row[hyper_cols].values))
                processed_keys[sf] = keys
                results[sf] = df_existing.to_dict('records')

    for curr_emb in tqdm(embs_list, desc="Evaluating embeddings"):
        info = curr_emb["info"]
        ordered_keys = sorted([k for k in info
                               if k not in ['checkpoint','epoch_num','early_stop_epoch','recall_topk']])
        info_tuple = tuple(info[k] for k in ordered_keys)

        all_done = True
        if resume and out_prefix:
            for sf in sample_fractions:
                if sf not in processed_keys or info_tuple not in processed_keys[sf]:
                    all_done = False
                    break
        else:
            all_done = False
        ...
        for metrics in res:
            sf = metrics["sample_fraction"]
            metrics_flat = {k: v for k, v in metrics.items() if k != "sample_fraction"}
            result_row = {**info, **metrics_flat}
            results[sf].append(result_row)

        if out_prefix:
            for sf, result_list in results.items():
                df = pd.DataFrame(result_list)
                csv_path = f"{out_prefix}_{sf:.3f}".rstrip("0").rstrip(".") + ".csv"
                df.to_csv(csv_path, index=False)

        if time_limit and (time() - start_time) > time_limit:
            print(f"Time limit ({time_limit} с). Saving and exiting.")
            break
